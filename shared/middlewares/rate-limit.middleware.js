...
const redisClient = require("../libraries/cache/redis.client");
...
class RateLimitMiddleware {
  ...
  createRateLimit(options) {
    return (req, res, next) => {
      ...
      const defaultOptions = {
        store, // Use Redis store if available, otherwise defaults to memory store
      ...
      try {
        ...
        rateLimit({ ...defaultOptions, ...options })(req, res, next);
      } catch (error) {
  ...
  authRateLimit() {
    return this.createRateLimit({
      ...
      keyGenerator: (req) => {
        ...
        return `rl:${this.serviceName}:auth:${getIp(req)}:${userId}`;
      },
  ...
  registerRateLimit() {
    return this.createRateLimit({
      ...
      keyGenerator: (req) => {
        return `rl:${this.serviceName}:register:${getIp(req)}`;
      },
  ...
  loginRateLimit() {
    return this.createRateLimit({
      ...
      keyGenerator: (req) => {
        return `rl:${this.serviceName}:login:${getIp(req)}:${
  ...
  otpRateLimit() {
    return this.createRateLimit({
      ...
      keyGenerator: (req) => {
        return `rl:${this.serviceName}:otp:${getIp(req)}:${
  ...
  passwordResetRateLimit() {
    return this.createRateLimit({
      ...
      keyGenerator: (req) =>
        `rl:${this.serviceName}:pwd_reset:${getIp(req)}:${
          req.body.phone || "unknown"
        }`,
  ...
  profileUpdateRateLimit() {
    return this.createRateLimit({
      ...
      keyGenerator: (req) =>
        `rl:${this.serviceName}:profileUpdate:${
          req.user?.userId || getIp(req)
        }`,
  ...
  fileUploadRateLimit() {
    return this.createRateLimit({
      ...
      keyGenerator: (req) =>
        `rl:${this.serviceName}:upload:${req.user?.userId || getIp(req)}`,
  ...
  adminRateLimit() {
    return this.createRateLimit({
      ...
      keyGenerator: (req) =>
        `rl:${this.serviceName}:admin:${req.user?.userId || getIp(req)}`,
  ...
  sessionRateLimit() {
    return this.createRateLimit({
      ...
      keyGenerator: (req) =>
        `rl:${this.serviceName}:session:${req.user?.userId || getIp(req)}`,
  ...
  settingsRateLimit() {
    return this.createRateLimit({
      ...
      keyGenerator: (req) =>
        `rl:${this.serviceName}:settings:${req.user?.userId || getIp(req)}`,
  ...
  accountActionRateLimit() {
    return this.createRateLimit({
      ...
      keyGenerator: (req) =>
        `rl:${this.serviceName}:account:${req.user?.userId || getIp(req)}`,
  ...
  createUserRateLimit(windowMs, max, prefix = "user") {
    return this.createRateLimit({
      ...
      max,
      keyGenerator: (req) =>
        `rl:${this.serviceName}:${prefix}:${req.user?.userId || getIp(req)}`,
  ...
  createBurstRateLimit(burstMax, sustainedMax, windowMs) {
    return async (req, res, next) => {
      try {
        ...
        const key = req.user?.userId || getIp(req);
        const now = Date.now();
    ...
    return async (req, res, next) => {
      try {
        ...
        const fallbackLimiter = this.createDistributedRateLimit({
          max: baseMax,
          windowMs,
          keyGenerator: () => req.user?.userId || getIp(req),
        });
