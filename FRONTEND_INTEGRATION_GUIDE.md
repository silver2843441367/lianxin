...
class _LoginScreenState extends State<LoginScreen> {
  ...
  void _handleLogin() async {
    ...
    try {
      if (_useOtpLogin) {
        // Send OTP for login
        await authProvider.requestLoginOtp(_phoneController.text);
...
class ApiClient {
  ...
  ApiResponse<T> _handleResponse<T>(Response response, T Function(dynamic)? fromJson) {
    ...
    try {
      ...
      final T? data = fromJson != null && responseData != null
  ...
  void onRequest(RequestOptions options, RequestInterceptorHandler handler) async {
    ...
    if (requiresAuth) {
      ...
      if (token != null) {
        ...
        if (expiry != null && expiry.isBefore(now.add(const Duration(minutes: 5)))) {
